...
![Mockup](https://zh-hans.reactjs.org/static/thinking-in-react-mock-1071fbcc9eed01fddc115b41e193ec11-4dd91.png)
...
![Component diagram](res/thinking-in-react-components-eb8bda25806a89ebdc838813bdfa3601-82965.png)
...
在==构建应用的静态版本时==，我们需要创建一些会重用其他组件的组件，然后==通过 *props* 传入所需的数据==。*props* 是父组件向子组件传递数据的方式。即使你已经熟悉了 *state* 的概念，也**完全不应该使用 state** 构建静态版本。state 代表了随时间会产生变化的数据，应当仅在实现交互时使用。所以构建应用的静态版本时，你不会用到它。
